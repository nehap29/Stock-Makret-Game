...
def draw(day):
        ...
        frame = Frame(master)
        # Create 2 buttons
        self.button_left = Tkinter.Button(frame,text="Next day",
                                        command=self.change_day)
        self.button_left.pack()
        fig = Figure()
        ax = fig.add_subplot(111)
        coords = graph_points(day)
        ax.plot(coords[0],label='comp1')
        ax.plot(coords[1],label='comp2')
        ax.plot(coords[2],label='comp3')
        ax.plot(coords[3],label='comp4')
        ax.plot(coords[4],label='comp5')
        ax.plot(coords[5],label='comp6')
        ax.plot(coords[6],label='comp7')
