...
class LogIn(Frame):
    ...
    def pass_enter(self,username):
        ...
        if foundpass == True:
            ...
            #pop up that displays the instructions
...
##widgets for news_frame
        ...
        #sets the most resent hint to be displayed 
...
##widgets for bank_frame
        ...
        quote = ('you have\n'+ str(money))
        v.set(quote)
        #Adding a label to display the banks interest rate that day
...
##Functions for the news_frame
    ...
    def present(self):
        ...
        #working out if they are on a day and difficulty where they get a new hint
        if (day-1) * level <= (len(img)-1) and (day%level == 0):
            self.i = (day-1) * level
            self.igm = PhotoImage(file=img[self.i])
            pic = Button(self.news_frame, image=self.igm)
...
##Increments the number owned list and updates the gui
    ...
    #function to work out the interest rate to be displayed inthe bank frame 
    def interest_stats():
        global interest_gain
        #fetching the interest rate for that day from the database
        interest = DB.interest_fetch()
        #calculating the compound interest
        interest_gain = int(round(money * interest))

    #function to pass all the required information to the databse class in order to saev the users progress
    # must go through the mian class as improtant variables that need to be updated are stored as local variables in the main class
    #variables suhc as no_owned list
    def info4save(self):
        global money
        global day
...
#function to fetch all the prices of the stocks on the current day to display in stats frame 
    def today_price():
        import sqlite3
        conn = sqlite3.connect('stock.db')
        c = conn.cursor()
...
#Funciotn to upsate users attributes in the database with the global variables in the program as a way of saving users progress
...
#the class incharge of running the separate graph window        
class Graph():
...
#code that will bring up the points that need to be plotted for each comapny day by day
    def graph_points(self):
        global day
        import sqlite3
        conn = sqlite3.connect('stock.db')
        ...
        #i is the company_ID
        for i in range(0,7):
            #have to do i+1 win the SQL command due to issues with the first record of the database 
            command = ("SELECT price FROM stock_price WHERE company_ID = '"+ str(i+1) +"'")
            c.execute(command)
            all_prices = c.fetchall()
